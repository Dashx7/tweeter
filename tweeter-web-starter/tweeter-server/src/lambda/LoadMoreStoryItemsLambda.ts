...
export let handler = async (event: LoadMoreStatusItemsRequest): Promise<LoadMoreStatusItemsResponse> => {
    ...
    return await performErrorReportingOperation(async () => {
        return new LoadMoreStatusItemsResponse(true, ...(await new StatusService().loadMoreStoryItems(event.authToken, event.user, event.pageSize, !!event.lastItem ? Status.fromJson(JSON.stringify(event.lastItem)) : null)));
    });
}