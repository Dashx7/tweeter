...
export class Status {
    ...
    public static fromJsonAsList(json: string | null | undefined): Status[] | null {
        if (!!json) {
            let statusList: Status[] = [];

            (JSON.parse(json) as Status[])?.forEach(jsonStatus => {
                const status = Status.fromJson(JSON.stringify(jsonStatus));
                if (status !== null) {
